...
type RecipeChatMessage = { sender: string, content: string, recipe?: Recipe, createdAt?: string };
...
export class RecipeChatService {
    ...
    private static readonly MAX_CACHED_MESSAGES = 80;
    private static readonly MAX_CACHED_MESSAGE_AGE_MS = 30 * 24 * 60 * 60 * 1000;
    ...
    private restoreMessages(): void {
        try {
            ...
            if (Array.isArray(parsed)) {
                this.messages.set(this.trimMessages(parsed
    ...
    private trimMessages(messages: RecipeChatMessage[]): RecipeChatMessage[] {
        const cutoff = Date.now() - RecipeChatService.MAX_CACHED_MESSAGE_AGE_MS;
        return messages
            .filter(message => {
                const timestamp = Date.parse(message.createdAt ?? '');
                return Number.isFinite(timestamp) && timestamp >= cutoff;
            })
            .slice(-RecipeChatService.MAX_CACHED_MESSAGES);
    }
