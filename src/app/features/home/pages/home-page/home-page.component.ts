...
@Component({
  ...
  styles: [`
    ...
    .mobile-coming-section { position: relative; }
    ...
    .mobile-mockup-stage { position: relative; min-height: 500px; display: flex; align-items: center; justify-content: center; overflow: visible; isolation: isolate; }
    .mobile-mockup-stage::before { content: ""; position: absolute; width: min(520px, 82vw); height: min(520px, 82vw); border-radius: 50%; background: radial-gradient(circle, rgba(22,198,90,0.2), rgba(245,158,11,0.1) 44%, rgba(255,255,255,0) 70%); transform: translate(6%, 3%); }
    .mobile-mockup-image { position: relative; z-index: 1; width: min(104%, 680px); max-width: none; display: block; filter: drop-shadow(0 28px 38px rgba(11,24,18,0.16)); }
    @media (min-width: 1024px) {
      .mobile-mockup-image { width: min(116%, 780px); transform: translateX(1.25rem) scale(1.02); }
    }
    @media (max-width: 767px) {
      ...
      .mobile-mockup-stage { min-height: auto; margin-top: 1.5rem; }
      .mobile-mockup-image { width: min(108%, 480px); filter: drop-shadow(0 18px 24px rgba(11,24,18,0.14)); }
    }
