...
import { MealPlanService } from '../../../meal-planner/services/meal-plan.service';
...
export class DashboardPageComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.dashboardAnalyticsService.fetchAnalytics().subscribe({
            next: () => this.prefetchLikelyNextScreens()
        });
        this.badgeService.fetchBadges().subscribe();
    }

    private prefetchLikelyNextScreens(): void {
        if (this.shouldAvoidPrefetch()) {
            return;
        }

        this.nutritionService.getTodayData().subscribe({ error: () => undefined });
        this.mealPlanService.getCurrentPlan().subscribe({
            next: () => this.shoppingListService.getLatest().subscribe({ error: () => undefined }),
            error: () => undefined
        });
        this.subscriptionService.getPlans().subscribe({ error: () => undefined });
    }

    private shouldAvoidPrefetch(): boolean {
