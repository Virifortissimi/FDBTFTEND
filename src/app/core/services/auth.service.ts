...
export class AuthService {
  ...
  private static readonly AUTH_PUBLIC_KEY_PLATFORM = 'web';
  private static readonly AUTH_PUBLIC_KEY_CACHE_KEY = `foodbot.auth.publicKey.v1.${AuthService.AUTH_PUBLIC_KEY_PLATFORM}`;
  private static readonly AUTH_PUBLIC_KEY_ALGORITHM = 'RSA-OAEP-256';
  private static readonly AUTH_PUBLIC_KEY_SIGNATURE_ALGORITHM = 'RSASSA-PKCS1-v1_5-SHA256';
  ...
  private async buildEncryptedLoginPayload(email: string, password: string) {
    ...
    return {
      ...
      platform: AuthService.AUTH_PUBLIC_KEY_PLATFORM,
  ...
  private async getAuthPublicKey(): Promise<AuthPublicKeyResponse> {
    ...
    const persistedPublicKey = await this.readPersistedPublicKey();
    if (persistedPublicKey && !this.isPublicKeyExpired(persistedPublicKey)) {
      this.cachedPublicKey = persistedPublicKey;
    ...
    this.publicKeyPromise = firstValueFrom(
      this.http.get<{ success: boolean; data: AuthPublicKeyResponse }>(
        `${this.apiUrl}/public-key?platform=${AuthService.AUTH_PUBLIC_KEY_PLATFORM}`
      )
    ).then(async res => {
      if (!res?.success || !res?.data) {
      ...
      const publicKey = this.normalizePublicKeyResponse(res.data);
      if (!(await this.verifyRuntimePublicKey(publicKey))) {
        throw new Error('Received an unverifiable auth public key.');
      }

      this.cachedPublicKey = publicKey;
      this.persistPublicKey(publicKey);
      return publicKey;
    }).catch(error => {
      const fallbackPublicKey = this.getFallbackPublicKey();
      if (fallbackPublicKey) {
        this.cachedPublicKey = fallbackPublicKey;
  ...
  private isPublicKeyExpired(publicKey: AuthPublicKeyResponse): boolean {
    if (publicKey.algorithm !== AuthService.AUTH_PUBLIC_KEY_ALGORITHM) {
      return true;
    }

    if (publicKey.platform !== AuthService.AUTH_PUBLIC_KEY_PLATFORM) {
      return true;
    }
  ...
  private clearCachedPublicKey() {
    ...
    this.publicKeyPromise = null;
    if (isPlatformBrowser(this.platformId)) {
      localStorage.removeItem(AuthService.AUTH_PUBLIC_KEY_CACHE_KEY);
    }
  }

  private async readPersistedPublicKey(): Promise<AuthPublicKeyResponse | null> {
    if (!isPlatformBrowser(this.platformId)) {
      return null;
    }

    const raw = localStorage.getItem(AuthService.AUTH_PUBLIC_KEY_CACHE_KEY);
    if (!raw) {
      return null;
    }

    try {
      const publicKey = this.normalizePublicKeyResponse(JSON.parse(raw));
      if (!(await this.verifyRuntimePublicKey(publicKey))) {
        localStorage.removeItem(AuthService.AUTH_PUBLIC_KEY_CACHE_KEY);
        return null;
      }
  ...
  private persistPublicKey(publicKey: AuthPublicKeyResponse) {
    if (!isPlatformBrowser(this.platformId)) {
      return;
    }

    localStorage.setItem(AuthService.AUTH_PUBLIC_KEY_CACHE_KEY, JSON.stringify(publicKey));
  }

  private normalizePublicKeyResponse(value: any): AuthPublicKeyResponse {
    const publicKey: AuthPublicKeyResponse = {
    ...
    if (!publicKey.keyId
      || publicKey.algorithm !== AuthService.AUTH_PUBLIC_KEY_ALGORITHM
      || publicKey.platform !== AuthService.AUTH_PUBLIC_KEY_PLATFORM
      || !publicKey.publicKeySpkiBase64
      || Number.isNaN(Date.parse(publicKey.issuedAtUtc))
      || Number.isNaN(Date.parse(publicKey.expiresAtUtc))) {
      throw new Error('Received an invalid auth public key.');
    }

    return publicKey;
  }

  private getFallbackPublicKey(): AuthPublicKeyResponse | null {
    const configuredFallback = environment.authEncryptionFallback;
    if (!configuredFallback) {
      return null;
    }

    try {
      const publicKey = this.normalizePublicKeyResponse({
        issuedAtUtc: new Date(0).toISOString(),
        ...configuredFallback
      });
      return this.isPublicKeyExpired(publicKey) ? null : publicKey;
    } catch {
      return null;
    }
  }

  private async verifyRuntimePublicKey(publicKey: AuthPublicKeyResponse): Promise<boolean> {
    const signingPublicKey = environment.authSigningPublicKeySpkiBase64?.trim();
    if (!signingPublicKey) {
      return true;
    }

    if (publicKey.signatureAlgorithm !== AuthService.AUTH_PUBLIC_KEY_SIGNATURE_ALGORITHM
      || !publicKey.signature) {
      return false;
    }
    ...
    try {
      const cryptoKey = await window.crypto.subtle.importKey(
        'spki',
        this.base64ToBytes(signingPublicKey),
        { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' },
        false,
        ['verify']
      );
      const signatureBytes = this.base64ToBytes(publicKey.signature);
      const payloadBytes = new TextEncoder().encode(this.buildPublicKeySignaturePayload(publicKey));
      return window.crypto.subtle.verify('RSASSA-PKCS1-v1_5', cryptoKey, signatureBytes, payloadBytes);
    } catch {
      return false;
    }
  }

  private buildPublicKeySignaturePayload(publicKey: AuthPublicKeyResponse): string {
    return [
      publicKey.keyId.trim(),
      publicKey.algorithm.trim(),
      publicKey.platform.trim().toLowerCase(),
      publicKey.publicKeySpkiBase64.trim(),
      publicKey.issuedAtUtc.trim(),
      publicKey.expiresAtUtc.trim()
    ].join('\n');
  }
  ...
  private isStaleEncryptedPayloadError(message: string): boolean {
    ...
    return normalized.includes('invalid login payload')
      ...
      || normalized.includes('expired or unknown')
