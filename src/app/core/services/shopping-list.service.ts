...
export class ShoppingListService {
    ...
    addItem(input: { name: string; quantity: string; category: string }): Observable<any> {
        return defer(() => {
            ...
            const optimisticItem: ShoppingItem = {
                ...
                name: input.name,
                quantity: input.quantity || '1 item',
                category: input.category || 'Other',
                emoji: this.categoryLabel(input.category),
                isChecked: false
            };

            this.setLocalList(this.withAddedItem(previousList, optimisticItem));
            ...
            return this.http.post<any>(`${this.apiUrl}/items`, input).pipe(
                ...
                catchError(error => {
                    this.setLocalList(previousList);
    ...
    updateItem(itemId: string, input: { name: string; quantity: string; category: string; isChecked?: boolean }): Observable<any> {
        return defer(() => {
            const previousList = this.currentList();
            this.updateLocalItem(itemId, {
                name: input.name,
                quantity: input.quantity || '1 item',
                category: input.category || 'Other',
                emoji: this.categoryLabel(input.category),
                isChecked: input.isChecked ?? previousList?.items.find(item => item.id === itemId)?.isChecked ?? false
            });
    ...
    private withAddedItem(list: ShoppingList | null, item: ShoppingItem): ShoppingList {
        const baseList = list ?? {
            id: '',
        ...
        return {
            ...baseList,
            items: [...baseList.items, item]
        };
    }

    private setLocalList(list: ShoppingList | null): void {
        this.currentList.set(list);
        this.cache.set(this.latestCacheKey, { success: true, data: list }, this.latestCacheTtlMs);
